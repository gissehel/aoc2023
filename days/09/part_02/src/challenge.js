...
const find_prev = (sequence) => {
    ...
    return sequences.map((sequence, index) => sequence[0]*((index)%2 === 0 ? 1 : -1)).reduce((a,b)=>a+b,0)
}
...
const challenge = (input) => {
    ...
    const result = sequences.map(sequence => find_prev(sequence)).reduce((a,b)=>a+b,0)
    return `${result}`;
}
